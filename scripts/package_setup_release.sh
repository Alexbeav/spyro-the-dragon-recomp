...
#
# Writes: dist/spyro-the-dragon-recomp-<VERSION>-<artifact-tag>.zip
set -euo pipefail
...
exec bash "${PACKAGER}" \
  ...
  --zip-prefix spyro-the-dragon-recomp \
  --exe-name Spyro_the_Dragon_Recompiled \
  --display-name "Spyro the Dragon Recompiled" \
  --recompiler-build "${RECOMPILER_BUILD}" \
  --version-env RELEASE_VERSION \
  --disc-hint "your legally owned Spyro the Dragon disc" \
